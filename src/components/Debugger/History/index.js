...
export default connect({
  ...
  function History ({
    ...
    const filteredHistory = history.filter(record => record.data.type === 'mutation' || showProvidersInHistory)
    const counts = history.reduce((currentCounts, record) => {
      return {
        mutations: record.data.type === 'mutation' ? currentCounts.mutations + 1 : currentCounts.mutations,
        providers: record.data.type !== 'mutation' ? currentCounts.providers + 1 : currentCounts.providers
      }
    }, {
      mutations: 0,
      providers: 0
    ...
    return (
      <div className='mutations'>
          <h3 className='mutations-title'>
            <span className='mutations-stat-mutations'>Mutations </span>: {counts.mutations}
            <span className='mutations-stat-divider'>|</span>
            <span className='mutations-stat-providers'>Providers </span>: {counts.providers}
            <div className='mutations-settingsContainer'>
              <label>providers: <input type='checkbox' onChange={() => showProvidersInHistoryToggled()} checked={showProvidersInHistory} /></label>
            </div>
          </h3>
        <div className='list'>
          {filteredHistory.map((record, index) => {
            ...
            const isSearchHit = searchValue &&
              record.data.type === 'mutation' && record.data.args[0].join('.').indexOf(searchValue) >= 0
            ...
            return (
              ...
              >
                ...
                <div
                  ...
                  onClick={() => signalClicked({executionId: record.executionId})}
                >
                  {record.signalName}
                </div>
                {record.data.type === 'mutation' ? (
                  <div className='list-content'>
                    <Mutation mutation={record.data} onMutationClick={(path) => mutationClicked({path})} pathClicked={pathClicked} />
                  ...
                  <Service showReturnValue service={record.data} pathClicked={pathClicked} />
                )}
              </li>
            )
          })}
