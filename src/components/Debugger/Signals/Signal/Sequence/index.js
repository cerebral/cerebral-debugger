...
class Sequence extends Component {
  ...
  render() {
    ...
    if (this.props.sequence.name) {
      containerStyle = {
        flexBasis: '30px',
        backgroundColor: nameToColors(
          this.props.sequence.name,
          this.props.sequence.name,
          0.9,
          0.2
        ).backgroundColor,
        color: nameToColors(this.props.sequence.name, this.props.sequence.name)
          .backgroundColor,
      }
    } else {
      containerStyle = {
    ...
    return (
      ...
      >
          ...
          >
            ...
            {this.state.showName ? (
              <span
                ...
                style={{
                  left: this.state.namePosition.x + 'px',
                  top: this.state.namePosition.y + 'px',
                }}
              >
                {this.props.sequence.name || this.props.sequence.type}
              </span>
        ...
        <div className="signal-sequenceContainer">{this.props.children}</div>
      </div>
    )
  }
}
